//******************************************************************************
//! @file $RCSfile: main_can_it_spy_echo_example.c,v $
//!
...
//!
//! @brief CAN spy displaying CAN activity on UART's, CAN and UART messages
//!        are interrupt handled
//!
...
//  @fn main
//!
//! Core of "main_can_it_spy_echo_example.c".
//!
...
//--- I N I T
    ...
    uart_mini_printf ("-0- ... No Activity on CAN Bus ! ...\r");
        ...
        // Init UART-1 at 19200 bauds
    Uart_select(UART_1);
    uart_init(CONF_8BIT_NOPAR_1STOP,38400);
        
    uart_mini_printf ("\r\n\n-1- ======= CAN SPY ECHO EXAMPLE ======= -1-\r\n");
    uart_mini_printf ("%s",txt_logo_atmel);
    if(mode==0) uart_mini_printf ("-1 %s 1-\r\n\n",txt_can_ana_2);
    else uart_mini_printf ("-1 %s 1-\r\n\n",txt_can_ana_1);
...
//---------------------------------------------------------------------------
    ...
    //- CAN ECHO: WAIT FOR RECEIVED
        ...
        while(1)
        {
            u8_temp = message.status;
            if (u8_temp != MOB_PENDING) break; // Out of while
        }
        if (u8_temp & ERR_MOB_MSK) break; // Error, out of the function
    ...
    //- CAN ECHO: RESPONSE
        ...
        if(mode==0)
        {
            ...
            // ---- Exit if CAN error(s)
            if (u8_temp & ERR_MOB_MSK) break; // Error, out of function
        }
    }
    can_error_mode();
    return 0;
...
//- UART ECHO
    ...
    while(1)
    {
        ...
        if (uart_test_hit())
        {
            uart0_putchar(uart0_getchar());   // Echo on UART-0
        }
        Uart_select(UART_1);
        if (uart_test_hit())
        {
            uart1_putchar(uart1_getchar());   // Echo on UART-1
        }
    }
}
