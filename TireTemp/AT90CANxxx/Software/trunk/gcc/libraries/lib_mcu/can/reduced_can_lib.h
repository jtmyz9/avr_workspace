//******************************************************************************
//! @file $RCSfile: reduced_can_lib.h,v $
//!
...
//!        library of reduced functions (vs can_lib) of:
...
// ----------
// @brief This union defines a CAN identifier mask and allows to access it
// in mode standart or throught a table.
typedef union{
  U16 std;
  U8  tab[2];
} can_msk_t;
...
// 4) msk   :  initialize by the application in reception.
// 5) dlc   :  initialize by the application to give the number of data to
//             transmit complete by the library in reception.
// 6) pt_data: pointer on the table which contains the data to send or
//             received.
// 7) status:  manage by the library.
typedef  struct{
  U8         handle;
  can_cmd_t  cmd;
  can_id_t   id;
