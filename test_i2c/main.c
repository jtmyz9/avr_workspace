...
Copyright (C) 2005, 2006 by Donald R. Blake
...
Change Activity:
   ...
   ------      -------------
  05 Sep 2005  Created.
  26 Sep 2005  Fixed date10 field in ds1307Reg_t structure typedef.
  22 Oct 2005  Fixed SIG_OUTPUT_COMPARE2 interrupt handler comment.
               Removed initialization of TWBR in main (it's done in twiInit).
  25 Mar 2006  Modifications for WINAVR 20060125.  Use FDEV_SETUP_STREAM instead
               of fdevopen( ).
               Added help text for "y - set day".
  22 May 2006  Fixed twiWrite byte count in setDay, outputControl and
               squareWaveControl.  Thanks Chris!
...
)
{
  ...
  static char        months[ 12 ][ 4 ] PROGMEM = {
       { "Jan" }, { "Feb" }, { "Mar" }, { "Apr" },
       { "May" }, { "Jun" }, { "Jul" }, { "Aug" },
       { "Sep" }, { "Oct" }, { "Nov" }, { "Dec" }
  };
...
)
{
  ...
  static FILE myStdIo = FDEV_SETUP_STREAM( uartPutChar, uartGetChar, _FDEV_SETUP_RW );
